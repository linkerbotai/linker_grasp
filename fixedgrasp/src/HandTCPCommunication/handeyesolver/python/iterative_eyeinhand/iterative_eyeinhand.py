...
class IterativeEyeinhandSolver:

    def calculate_result(hand_data, eye_data):
        y_pose = IterativeEyeinhandSolver.AXXB(hand_data, eye_data)
        ...
        x_pose = IterativeEyeinhandSolver.AXXB(hand_data_inverse_list, eye_data_inverse_list)
    ...
    def AXXB(AXXB_fk_list,AXXB_ndi_list):
        ...
        if nums >= 4:
            ...
            B = np.array([
                np.mat(AXXB_ndi_list[i + 1]) @ (np.mat(AXXB_ndi_list[i]).I) for i in range(nums - 1)
            ])
            ...
            Rx, tx = IterativeEyeinhandSolver.find_rotation_and_translation(rotationA[0:nums - 2], rotationB[0:nums - 2], translationA[0:nums - 2], translationB[0:nums - 2])
            X = np.eye(4)
            X[0:3, 0:3] = Rx
            ...
            for i in range(0):
                ...
                for j in range(times):
                    dis = IterativeEyeinhandSolver.ErrorEst(marker_flange_data, rotA[j], rotB[j], tA[j], tB[j])
                    E1.append(dis)
                E1 = np.array(E1)
                m1 = np.median(E1)
                ...
                size = loopA.shape[0]
                print(size)
                Rx, tx =IterativeEyeinhandSolver.find_rotation_and_translation(loopA[0:size - 2], loopB[0:size - 2], ltA[0:size - 2], ltB[0:size - 2])
    ...
    def find_rotation_and_translation(RA, RB, translationA, translationB):
        ...
        for i in range(RA.shape[0]):
            V1i = IterativeEyeinhandSolver.findrotationaxis(RA[i])
            V1.append(V1i)
            V2i = IterativeEyeinhandSolver.findrotationaxis(RB[i])
            V2.append(V2i)
        V1 = np.array(V1)
        V2 = np.array(V2)
        A = np.zeros((4, 4))
        for i in range(V1.shape[0]):
            Ci = IterativeEyeinhandSolver.Q(V1[i, :]) - IterativeEyeinhandSolver.W(V2[i, :])
            Ai = np.dot(Ci.T, Ci)
            A = A + Ai
        eig = np.linalg.eig(A)
        eigenvalue = eig[0]
...
if __name__ == "__main__":
    ...
    print(len(ndi_list))
    maker_flange_dat = IterativeEyeinhandSolver.AXXB(fk_list, ndi_list)
